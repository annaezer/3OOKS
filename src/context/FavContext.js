...
function FavContextProvider({children}) {
    ...
    // I want to get the saved favourites out of the Local Storage and otherwise start with an empty array
    const [fav, setFav] = useState(JSON.parse(localStorage.getItem("favourite books")) || []);

    // Creating a function for adding the favourites and making sure it's not added twice by comparing unique key (=primary_isbn10) with testing function some
    function addFav(book) {
        ...
        } if (!fav.some(test => test["id"] === book.id)) {
            setFav([...fav, book]);
            toast.success("Added to Favourites", {
                icon: <span className="material-symbols-outlined">favorite</span>,
            })
